...
- Broad first-session inspection uses `get_workspace_overview`, whose per-tool OAuth metadata requests all role-compatible inspection scopes together. Admins receive workspace, usage, connections, and pipelines access in one consent; viewers and operators omit the admin-only `usage:read` scope and receive an explicit restricted-capacity marker instead of an authorization loop. Granular tools keep their narrow individual scope contracts.
