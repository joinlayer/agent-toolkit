...
2. For a broad first-session inspection, call `get_workspace_overview`. It requests all read-only scopes available to the authenticated role together and returns workspace identity, role-visible capacity, connections, and pipelines in one result. For viewers and operators, report that capacity is admin-only when the result marks it restricted; do not retry or request `usage:read`. State the workspace name/ID, agent identity, role, and granted scopes; stop if the workspace is not the one the user expects.
